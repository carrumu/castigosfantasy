...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Calendario de LaLiga (ver _shared/laligaCalendar.ts): dice cuando una
    // jornada ha cerrado de verdad. No da puntos por jornada, solo el
    // acumulado en /standings, asi que sirve para avisar "ha terminado la
    // Jornada X", no para adivinar quien quedo ultimo.
    const finishedMatchdays = await fetchFinishedMatchdays();
    console.log(`[comunio] community=${communityId} members=${members.length} standings=${standings.length} matchdays finished=${finishedMatchdays.length}`);
